                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.id} style={tbodyRow}>
                      ...
                      <td style={td}>{row.coordinator || "-"}</td>
                      <td style={td}>{row.team || "-"}</td>
                      <td style={td}>{row.isp || "-"}</td>
                      <td style={td}>
                        <span style={statusPill(row.status)}>{row.status || "pending"}</span>
                      </td>
                      <td style={td}>
                      ...
                      <td style={td}>
                        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                          <button onClick={() => updateStatus(row.id, "pending")} style={tinyBtn("#e5e7eb", "#111")}>
                            Pending
                          </button>
                          <button onClick={() => updateStatus(row.id, "approved")} style={tinyBtn("#16a34a", "white")}>
                            Approve
                          </button>
                          <button onClick={() => updateStatus(row.id, "needs_info")} style={tinyBtn("#eab308", "white")}>
                            Needs Info
                          </button>
                          <button onClick={() => updateStatus(row.id, "rejected")} style={tinyBtn("#dc2626", "white")}>
                            Reject
                          </button>
                        </div>
                      </td>
